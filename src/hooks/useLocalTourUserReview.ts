...
const STORAGE_KEY = "datviet.tourUserReviews";

export type StoredUserTourReview = {
  id: string;
  rating: number;
  comment: string;
  author: string;
...
type Store = Record<string, StoredUserTourReview>;
...
export function useLocalTourUserReview(slug: string) {
  ...
  const save = useCallback(
    (payload: { rating: number; comment: string; author: string }) => {
      ...
      const next: StoredUserTourReview = {
        id,
